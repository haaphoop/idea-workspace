 ...
 * implemented as instances of this class. 字符串类表示字符串。java程序中的所有字符串，如“ABC”，是实现这个类的实例。
 * <p>
 * Strings are constant; their values cannot be changed after they
 * are created. String buffers support mutable strings.
 * Because String objects are immutable they can be shared.
 * 字符串是常数；它们的值在创建之后不能更改。
...
public final class String
     ...
     * unnecessary since Strings are immutable.
     * 初始化新创建的 String对象，以便它表示与参数相同的字符序列；换句话说，新创建的字符串是参数字符串的副本。除非需要代码原始的显式拷贝，否则使用此构造函数是不必要的，因为字符串是不可变的。
     *
     * @param  original
     *         A {@code String}
     */
     ...
     * 分配一个新的字符串，以便它代表当前包含在字符数组参数中的字符序列。字符数组的内容被复制；字符数组的后续修改不会影响新创建的字符串。
     * @param  value
     ...
     * 从参数数组的子数组中分配一个新的字符串，以便它代表当前包含在字符数组参数中的字符序列。字符数组的内容被复制；字符数组的后续修改不会影响新创建的字符串。
     * @param  value
     ...
     * 提供此构造函数以便于迁移到StringBuilder。通过ToString方法从字符串生成器获得字符串可能运行得更快，通常是首选的。
     * @param   builder
